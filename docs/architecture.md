...
Ten categories, each with a modal reading in Boxes & Diamonds notation:

| Category | Modal Reading | Axis |
|---|---|---|
| `DEFINITE_EXISTENCE` | □X | existence |
| `PROBABLE_EXISTENCE` | ◇X | existence |
| `AMBIVALENT_EXISTENCE` | ◇X ∧ ◇¬X | existence |
| `PROBABLE_NEGATED_EXISTENCE` | ◇¬X | existence |
| `DEFINITE_NEGATED_EXISTENCE` | □¬X | existence |
| `INDICATION` | ¬K_a(X) ∧ ¬K_a(¬X) | epistemic |
| `HISTORICAL` | P(X) | temporality |
| `HYPOTHETICAL` | hyp(X) | temporality |
| `FAMILY` | fam(X) | experiencer |
| `UNRESOLVED` | ⊥ | — |

`INDICATION` is a first-class category (not present in pyConTextNLP or medspaCy) encoding that the clinician is actively investigating whether X is present — they have neither asserted nor denied it.
...
Twelve dataclasses mirroring the constructor subset of gamen-hs's 24-constructor JSON format. Each implements `to_tree_json()` (for the gamen-validate wire protocol) and `to_flat_extraction()` (for LLM-extracted formula output).

Implemented constructors: `Atom`, `Not`, `And`, `Or`, `Implies`, `Box`, `Diamond`, `Past`, `FutureBox`, `FutureDiamond`, `Knowledge`, `Indication`.
...
Converts `AssertionCategory` values to `ModalFormula` instances, binding the entity text as the propositional atom.
